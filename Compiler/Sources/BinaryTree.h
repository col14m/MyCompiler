...
public:
	...
	void DumpToGraph(char *logPNGname) const;
...
void BinaryTreeNode::DumpToGraph(char *logPNGname) const
{
	assert(logPNGname);

	FILE *log = NULL;

	char fullLogPNGname[40] = ".\\Logs\\";
...
void ConvertVGtoPNG(char *logPNGname)
{
	...
	FILE *nowBat = NULL;
	fopen_s(&nowBat, ".\\Logs\\toPNG.bat", "w");
	//printf("+++++++++++++\n");
	assert(nowBat);
	...
	fprintf(nowBat, "D:\\Graphviz2.38\\bin\\dot ");
	fprintf(nowBat, "-Tpng \"%%CD%%\\Logs\\%s\" ", fullInPNGname);
	fprintf(nowBat, "   -o \"%%CD%%\\Logs\\%s\" \n", fullOutPNGname);

	fclose(nowBat);
	//printf("++++++----------------+++++++\n");
	WinExec(".\\Logs\\toPNG.bat", SW_HIDE);
	//system("cls");
}
